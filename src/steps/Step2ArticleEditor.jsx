...
export default function Step2ArticleEditor({ data, onUpdate, onNext }) {
  ...
  var handleLoadOriginal = async function() {
    // 대상 조례 또는 참고 조례에서 원본 소스 결정
    var sourceOrdinance = data.targetOrdinance
    if (!sourceOrdinance && data.selectedRefs && data.selectedRefs.length > 0) {
      sourceOrdinance = data.selectedRefs[0]
      console.log('[Step2] targetOrdinance 없음, selectedRefs[0] 사용:', sourceOrdinance.name)
    }
    if (!sourceOrdinance) {
    ...
    setOrigLoading(true)
    console.log('[Step2] 원본 불러오기:', sourceOrdinance.name, ', serial:', sourceOrdinance.serial || sourceOrdinance.id)

    try {
      // 객체 전달 → serial로 직접 조회 (재검색 불필요)
    ...
    try {
      ...
      if (hasRefs) {
        ...
        for (var i = 0; i < data.selectedRefs.length; i++) {
          ...
          var fullText = await getOrdinanceFullText(ref)
          if (fullText) {
      ...
      <div className="step-footer">
        {(isAmend || hasRefs) && !origLoaded && (
          <button
            ...
            {origLoading
              ? '원본 불러오는 중...'
              : isAmend ? '원본 불러오기 (구조문)' : '참고 조례 불러오기'}
          </button>
        )}
        {(isAmend || hasRefs) && origLoaded && (
          <span style={{
